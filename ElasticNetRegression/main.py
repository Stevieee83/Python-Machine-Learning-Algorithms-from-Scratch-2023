...
from elasticnet_regression import ElasticNet
...
# Loads the train and testing datasets from the ElasticNet regression algorithm
X_train, X_test, y_train, y_test = data.standard_scale()

# Stores the input the parameters to the ElasticNet regression algorithm
parameters = {
    "l_ratio" : 0.5,
    "l" : 0.1,
    "lr" : 0.1,
...
# Creates the ElasticNet Regression algorithm and stores it in the model object
model = ElasticNet(**parameters)

# Fits the data to the ElasticNet regression algorithm during training
model.fit(X_train, y_train)

# Runs the ElasticNet regression model in inference mode and stores the predictions in the y_pred Python variable
